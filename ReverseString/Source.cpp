...
void ReverseString(char* stringToReverse)
{
	...
	char* stringEnd = stringToReverse;
	char tempChar = 0;

	while (*(stringEnd + sizeof(char)) != 0)
	{
		stringEnd++;
	}

	while (stringEnd != stringToReverse && stringEnd > stringToReverse)
	{
		tempChar = *stringToReverse;
		*stringToReverse = *stringEnd;
		*stringEnd = tempChar;
